...
public class CourseDAO extends DBContext {
    ...
    public ArrayList<Map<Lesson, Video>> getAllLessonByModuleid(int moduleid) throws Exception {
        ...
        ResultSet rs = null;
        try {
            prepare = connection.prepareStatement("select Module.module_id,Module.module_name,Lesson.lesson_id,Lesson.lesson_name,Lesson.status_lesson, Lesson.content_id,Lesson.video_id from Module\n"
            ...
            while (rs.next()) {
                ...
                Lesson les = new Lesson(rs.getInt(3), rs.getString(4), rs.getInt(1), (boolean) rs.getBoolean(5), rs.getInt(6), rs.getInt(7));
    ...
    public Lesson getLessonByVideoid(int videoid) throws Exception {
        ...
        try {
            ...
            rs = prepare.executeQuery();
            while (rs.next()) {
                less = new Lesson(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getBoolean(4), rs.getInt(5), rs.getInt(6));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
